...
async def _draw_electric_fish_result_impl(electric_data: Dict[str, Any]) -> Image.Image:
    ...
    total_h = 30 if (stolen_fish and total_value > 0) else 0
    y += total_h
    ...
    rate_text = f"本次成功率: {success_rate * 100:.1f}%"
    rw, _ = get_text_size(rate_text, small_font)
    draw.text(((width - rw) // 2, cur_y), rate_text, font=small_font, fill=text_secondary)
    cur_y += RATE_H
    ...
    if stolen_fish:
        ...
        if total_value > 0:
            total_text = f"总价值: {total_value} 金币"
            tvw, _ = get_text_size(total_text, small_font)
            draw.text(((width - tvw) // 2, cur_y), total_text, font=small_font, fill=gold_color)
            cur_y += 30
...
def _create_electric_fallback_image(electric_data: Dict[str, Any]) -> Image.Image:
    ...
    draw.text((50, 100), f"{electric_data.get('thief_name', '?')} → {electric_data.get('victim_name', '?')}", font=content_font, fill=primary_dark)
    draw.text((50, 140), electric_data.get('message', '')[:100], font=content_font, fill=primary_dark)
    return image
