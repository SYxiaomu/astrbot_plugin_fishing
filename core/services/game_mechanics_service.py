...
class GameMechanicsService:
    ...
    def forecast_wipe_bomb(self, user_id: str) -> Dict[str, Any]:
        ...
        normal_ranges = [
            (0.0, 0.2, 8000),      # 严重亏损（降低权重）
            (0.2, 0.5, 15000),     # 普通亏损（降低权重）
            (0.5, 0.8, 12000),     # 小亏损（降低权重）
            (0.8, 1.2, 25000),     # 小赚（保持不变）
            (1.2, 2.0, 18000),     # 中赚（提高权重）
            (2.0, 3.0, 6000),      # 大赚（提高权重）
            (3.0, 6.0, 1000),      # 超大赚（提高权重）
            (6.0, 15.0, 200),      # 高倍率（提高权重）
            (15.0, 50.0, 50),      # 超级头奖（提高权重）
            (50.0, 200.0, 15),     # 传说级奖励（提高权重）
            (200.0, 1500.0, 3),    # 神话级奖励（提高权重）
        ]
    ...
    def handle_wof_timeout(self, user_id: str) -> Dict[str, Any] | None:
        ...
        if (now - user.wof_last_action_time).total_seconds() > timeout_seconds:
            ...
            message = f"[CQ:at,qq={user_id}] ⏰你的操作已超时，系统已自动为你结算\n💰当前奖金 {prize} 金币。"
    ...
    def start_wheel_of_fate(self, user_id: str, entry_fee: int) -> Dict[str, Any]:
        ...
        extra_attempts = 0
        boost_buff = self.buff_repo.get_active_by_user_and_type(
            user_id, "WHEEL_OF_FATE_ATTEMPTS_BOOST"
        )
        if boost_buff and boost_buff.payload:
            try:
                payload = json.loads(boost_buff.payload)
                ...
                logger.warning(f"解析命运之轮buff载荷失败: user_id={user_id}")

        total_max_attempts = wheel_of_fate_daily_limit + extra_attempts

        # 检查次数是否已达上限
        if user.wof_plays_today >= total_max_attempts:
            return {"success": False, "message": f"今天的运气已经用光啦！你今天已经玩了 {user.wof_plays_today}/{total_max_attempts} 次命运之轮，请明天再来吧。"}
        # --- 限制逻辑结束 ---
        ...
        if random.random() < success_rate:
            ...
            return {
                ...
                "message": (f"[CQ:at,qq={user_id}] 🎯 第 {user.wof_current_level} 层幸存！ (下一层成功率: {next_success_rate}%)\n"
                            ...
                            f"⏲️请在{config.get('timeout_seconds', 60)}秒内回复【继续】或【放弃】！")
            }
        else:
            # 失败
            ...
            return {
                ...
                "message": (f"[CQ:at,qq={user_id}] 💥 湮灭！ \n"
                            f"你在通往第 {user.wof_current_level + 1} 层的路上失败了\n失去了入场的 {lost_amount} 金币..."),
            }
    ...
    def cash_out_wheel_of_fate(self, user_id: str, is_final_win: bool = False) -> Dict[str, Any]:
        ...
        if is_final_win:
            message = (f"🏆 [CQ:at,qq={user_id}] 命运的宠儿诞生了！ \n"
                       f"你成功征服了命运之轮的10层\n最终赢得了 {prize} 金币的神话级奖励！")
        else:
            message = (f"✅ [CQ:at,qq={user_id}] 明智的选择！ \n"
                       f"你成功将 {prize} 金币带回了家\n本次游戏净赚 {prize - entry} 金币。")
